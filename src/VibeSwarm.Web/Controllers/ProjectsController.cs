...
public class ProjectsController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] Project project, CancellationToken ct)
    {
        ...
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("UNIQUE", StringComparison.OrdinalIgnoreCase) == true)
        {
            ...
            var error = inner.Contains("Projects", StringComparison.OrdinalIgnoreCase) &&
				...
				: "A duplicate value was detected. Check for duplicate provider selections, agent assignments, or environment names.";
	...
	[HttpGet("dashboard-running-jobs")]
	public async Task<IActionResult> GetDashboardRunningJobs(CancellationToken ct = default)
		=> Ok(await _projectService.GetDashboardRunningJobsAsync(ct));
}
