...
if (project != null)
{
_credentialService.PopulateForClient(project);
}
...
EncryptEnvironmentCredentials(project.Environments);
...
var existingEnvironmentsById = existing.Environments.ToDictionary(environment => environment.Id);
foreach (var environment in project.Environments)
{
existingEnvironmentsById.TryGetValue(environment.Id, out var existingEnvironment);
_credentialService.Encrypt(environment, existingEnvironment);
}
...
environment.Notes = string.IsNullOrWhiteSpace(environment.Notes)
? null
...
var defaultEnvironment = normalized.FirstOrDefault(environment => environment.IsDefault);
if (normalized.Count > 0 && defaultEnvironment == null)
{
normalized[0].IsDefault = true;
defaultEnvironment = normalized[0];
}

foreach (var environment in normalized)
{
environment.IsDefault = environment.Id == defaultEnvironment?.Id;
}
...
EncryptedUsername = requested.EncryptedUsername,
EncryptedPassword = requested.EncryptedPassword,
CreatedAt = requested.CreatedAt,
UpdatedAt = requested.UpdatedAt
});
continue;
...
current.IsDefault = requested.IsDefault;
current.IsEnabled = requested.IsEnabled;
current.SortOrder = requested.SortOrder;
current.EncryptedUsername = requested.EncryptedUsername;
current.EncryptedPassword = requested.EncryptedPassword;
current.UpdatedAt = DateTime.UtcNow;
}
}
...
{
if (!Uri.TryCreate(environment.Url, UriKind.Absolute, out var uri)
|| (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps))
{
throw new InvalidOperationException($"Environment '{environment.Name}' must use an absolute http or https URL.");
}
}
}

private void EncryptEnvironmentCredentials(ICollection<ProjectEnvironment> environments)
{
foreach (var environment in environments)
{
_credentialService.Encrypt(environment);
}
}

private void PopulateForClient(IEnumerable<Project> projects)
{
