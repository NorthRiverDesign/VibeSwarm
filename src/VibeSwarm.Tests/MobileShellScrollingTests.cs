...
public sealed class MobileShellScrollingTests
{
	...
	public void SiteCss_UsesMomentumScrollingForAppShell()
	{
		...
		Assert.DoesNotMatch(new Regex(@"body::after\s*\{[^}]*bottom:\s*0;[^}]*height:\s*var\(--vs-safe-area-bottom\);", RegexOptions.Multiline), css);
	}
	...
	public void FullHeightScreens_UseViewportSafeHeightRules()
	{
		...
		Assert.Matches(new Regex(@"@supports \(-webkit-touch-callout: none\)\s*\{[\s\S]*body:has\(\.app-layout\)\s*\{[\s\S]*height:\s*-webkit-fill-available;", RegexOptions.Multiline), css);
		Assert.Matches(new Regex(@"html\s*\{[\s\S]*background-color:\s*var\(--bs-body-bg\);", RegexOptions.Multiline), css);
	}
